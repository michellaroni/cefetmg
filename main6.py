...
class Formulario(QWidget):
                ...
                for atividade, itens in self.dados_atividades.items():
                    ...
                    for item in itens_filtrados:
                        texto_exibicao += (
                            ...
                            f"  Unidade de Cálculo: {item['horas']}\n"
                            f"  PDF: {item['pdf']}\n"
                            f"  Horas Convertidas: {item['horas_convertidas']:.2f} (Cálculo: {item.get('calc_details', '')})\n"
    ...
    def incluir_certificado(self):
        ...
        - Registra detalhadamente como foi o cálculo em 'calc_details'.
        """
        try:
            ...
            if not matricula or not unidade_calculo_str or not caminho_pdf:
                if label_unidade is not None:
                    self.mostrar_mensagem(
                        "Erro",
                        f"Os campos Matrícula, Unidade de Cálculo ({label_unidade}) e PDF são obrigatórios.",
                        "critical"
                    )
            ...
            if label_unidade is not None:
                try:
                    user_input_val = float(unidade_calculo_str)
                except ValueError:
            ...
            if calcHoras_valor is None:
                # Exemplo: "09. Visita Técnica" => K * user_input_val
                baseHoras = user_input_val
                calc_details_text = f"K={k_valor} x Unidade ({label_unidade})={user_input_val}"
            else:
                # Se label_unidade for None => K x calcHoras_valor
                # Se label_unidade for algo => K x calcHoras_valor x user_input_val
                if label_unidade is not None:
                    baseHoras = calcHoras_valor * user_input_val
                    calc_details_text = f"K={k_valor} x {calcHoras_valor} x Unidade ({label_unidade})={user_input_val}"
                else:
                    baseHoras = calcHoras_valor
                    calc_details_text = f"K={k_valor} x {calcHoras_valor}"

            # Multiplica por K (SEM usar maxAproveitamento)
            horas_convertidas = k_valor * baseHoras

            item = {
                "matricula": matricula,
                "horas": user_input_val,
                "pdf": caminho_pdf,
                ...
                # registra como foi o cálculo ao lado das horas convertidas
                "calc_details": calc_details_text
            }
        ...
        try:
            ...
            for atividade, itens in dados_temp.items():
                ...
                for item in itens_filtrados:
                    ...
                    pdf.cell(0, 10, f"Horas Convertidas: {item['horas_convertidas']:.2f} (Cálculo: {item.get('calc_details', '')})", ln=True)
